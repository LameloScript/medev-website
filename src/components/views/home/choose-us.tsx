export default function ChooseUs() {
  ...
  return (
    <section className="min-h-screen flex flex-col lg:gap-24 lg:py-22 py-14 px-4 lg:px-20">
      <div className="flex flex-row items-center justify-center px-4 gap-10">
        <div className="">
          <span className="text-black text-xs md:text-sm text-start ">
            [POURQUOI NOUS CHOISIR ?]
          </span>
          <h2 className="text-black font-bangers text-3xl md:text-4xl lg:text-5xl mt-4 md:mt-6 text-start lg:leading-14 leading-tight">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 md:gap-8 px-4 ">
        {items.map((item, idx) => (
          ...
          >
            <span className="font-bangers text-xl md:text-3xl lg:text-3xl leading-tight max-w-[18ch]">
              {item.title}
            </span>
            <img
              src={item.img}
              alt={item.title}
              className="absolute bottom-0 right-0 w-28 h-auto md:w-32 lg:w-36 object-contain"
            />
          </div>
        ))}
      </div>
